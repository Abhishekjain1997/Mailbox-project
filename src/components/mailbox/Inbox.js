...
const Inbox = () => {
  ...
  const getData = useCallback(async () => {
    try {
      setLoading(true);
      let res = await fetch(
        `https://mailbox-client-ac835-default-rtdb.firebaseio.com/${changedMail}Inbox.json`
      );
  ...
  const DeleteHandler = async (id) => {
    ...
    const res = await fetch(
      `https://mailbox-client-ac835-default-rtdb.firebaseio.com/${changedMail}Inbox/${id}.json`,
      {
        method: "DELETE",
        headers: {
